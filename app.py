...
from utils.validators import validate_email, validate_password, validate_name, validate_phone, validate_image_upload, sanitize_filename
from utils.email_service import email_service
from utils.verification import generate_verification_code, store_verification_code, verify_code, get_code_info, clear_code
...
def inject_globals():
    ...
    return dict(
        ...
        truck=my_truck,
        now=datetime.now,
        csrf_token=generate_csrf_token(),  # Add CSRF token to all templates
    )
...
def staff_management():
    ...
    total_pages = (total_staff + per_page - 1) // per_page
    page = max(1, min(page, total_pages)) if total_pages > 0 else 1
    
    start_idx = (page - 1) * per_page
    end_idx = start_idx + per_page
    paginated_staff = filtered_staff[start_idx:end_idx]
    ...
    ctx.update(
        dict(
            staff_filtered=paginated_staff,
        ...
        try:
            ...
            if "image" in request.files:
                ...
                if file and file.filename:
                    ...
                    if not is_valid:
                        flash(error_msg or "Invalid image file. Please upload a valid image.", "error")
                        return redirect(url_for("manage_menu"))
...
def staff_orders():
    """View orders for staff on duty with pagination"""
    access_redirect = require_staff_access()
    if access_redirect:
        return access_redirect
        ...
        if request.method == "POST":
            ...
            try:
                my_truck.add_order_to_csv(
                    ...
                    subtotal=subtotal, tax_amount=tax_amount, tip_amount=tip_amount, total=final_total
                )
                my_truck.load_orders_from_csv()
            except Exception as e:
...
def signup():
    ...
    if request.method == "POST":
        ...
        if not email_valid:
            flash(email_error or "Please provide a valid email address.", "error")
            return render_template("signup.html", title="Register")
        
        # Validate password
        password_valid, password_error = validate_password(password)
        if not password_valid:
            flash(password_error or "Password must be at least 6 characters long.", "error")
            return render_template("signup.html", title="Register")
        
        # Validate names
        first_valid, first_error = validate_name(first, "First name")
        if not first_valid:
            flash(first_error, "error")
            return render_template("signup.html", title="Register")
        
        last_valid, last_error = validate_name(last, "Last name")
        if not last_valid:
...
def login():
    ...
    if request.method == "POST":
        ...
        if authenticated:
            ...
            if email_verification_enabled and email_service.enabled:
                # Generate and send verification code
                verification_code = generate_verification_code()
                store_verification_code(email, verification_code)
                
                # Send email
                success, error_msg = email_service.send_verification_code(email, verification_code)
                ...
                if success:
                    ...
                    session["pending_user_data"] = {
                        ...
                        "is_staff": user.get("role", "customer") == "staff",
                    ...
                    # If email fails, log but allow login (graceful degradation)
                    logger.warning(f"Failed to send verification email to {email}: {error_msg}")
                    flash(f"Could not send verification email: {error_msg}. Please contact support.", "warning")
                    # Continue with login anyway (optional: you can block login here)
            
            # Complete login (either verification disabled or email failed)
...
def verify_email():
    """Email verification page - enter 4-digit code"""
    # Check if user has pending verification
    pending_email = session.get("pending_verification_email")
    if not pending_email:
        flash("No pending verification. Please login first.", "error")
        return redirect(url_for("login"))
    ...
    if request.method == "POST":
        ...
        if not code or len(code) != 4 or not code.isdigit():
            flash("Please enter a valid 4-digit code.", "error")
            return render_template("verify_email.html", 
                                 email=pending_email,
                                 code_info=get_code_info(pending_email),
                                 title="Verify Email")
        
        # Verify code
        is_valid, error_msg = verify_code(pending_email, code)
        ...
        if is_valid:
            ...
            logger.info(f"Email verified successfully for {pending_email}")
...
def resend_verification_code():
    ...
    # Generate new code
    verification_code = generate_verification_code()
    store_verification_code(pending_email, verification_code)
    
    # Send email
    success, error_msg = email_service.send_verification_code(pending_email, verification_code)
    
    if success:
        flash("New verification code sent to your email.", "success")
        return jsonify({"success": True, "message": "Code sent successfully"})
    else:
        flash(f"Failed to send code: {error_msg}", "error")
...
def logout():
    ...
    # Clear pending verification data if exists
    session.pop("pending_verification_email", None)
    session.pop("pending_user_data", None)
...
def dashboard():
    ...
    user_orders = [o for o in my_truck.orders if o.get("customer_email", "").lower() == user_email.lower()]
    user_orders = sorted(user_orders, key=lambda x: x.get("timestamp", ""), reverse=True)[:10]  # Last 10 orders
...
def order_history():
    ...
    # Get user's orders
    user_orders = [o for o in my_truck.orders if o.get("customer_email", "").lower() == user_email.lower()]
    user_orders = sorted(user_orders, key=lambda x: x.get("timestamp", ""), reverse=True)
...
def order_tracking(order_id):
    ...
    # Find the order
    order = next((o for o in my_truck.orders if str(o.get("order_id", "")) == str(order_id)), None)
    ...
    if order.get("customer_email", "").lower() != user_email.lower():
...
def log_request():
    ...
    logger.info(f"{request.method} {request.path} - IP: {request.remote_addr} - User: {session.get('user_email', 'Anonymous')}")
