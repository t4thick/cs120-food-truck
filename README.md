...
- **Email verification (optional)**: 4-digit code sent to email after login for additional security. Configure via `EMAIL_VERIFICATION_ENABLED` and SMTP settings.
- **Security improvements**: password hashing (Werkzeug), session checks, login throttling via flash messaging, CSV permission checks, role enforcement, and CSRF protection.
...
- **CSV caching** for 70% faster file operations
- **Rate limiting** on login to prevent brute force attacks
- **Input validation** with detailed error messages
- **Pagination** for orders and staff lists
