...
from utils.csv_cache import csv_cache
...
class FoodTruck:
    ...
    def load_menu_from_csv(self, path="data/menu.csv"):
        """Load menu items from CSV with caching"""
        # Check cache first
        cached_data = csv_cache.get_cached_data(path)
        if cached_data is not None:
            self.menu_items = cached_data
            return
        ...
        try:
            ...
            # Update cache
            csv_cache.update_cache(path, self.menu_items)
    ...
    def load_staff_from_csv(self, path="data/users.csv"):
        ...
        try:
            ...
            # Update cache
            csv_cache.update_cache(path, self.staff)
        except FileNotFoundError:
    ...
    def load_schedules_from_csv(self, path="data/schedules.csv"):
        ...
        try:
            ...
            # Update cache
            csv_cache.update_cache(path, self.schedules)
        except FileNotFoundError:
            pass
        ...
        try:
            ...
            with open(path, newline="", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    # Parse financial fields with defaults
                    try:
            ...
            # Update cache
            csv_cache.update_cache(path, self.orders)
    ...
    def add_order_to_csv(
        ...
        # Generate unique order ID using timestamp + UUID
        ...
        with open(path, "a", newline="") as f:
            ...
            writer.writerow(
                [
                    ...
                    str(subtotal),  # Store actual subtotal
                    str(tax_amount),  # Store actual tax
                    str(tip_amount),  # Store actual tip
                    str(total),  # Store actual total
                ]
            )
    ...
    def initialize_csv_files(self):
        ...
        files_config = {
            ...
            "data/orders.csv": ["Order_ID", "Customer_Name", "Customer_Email", "Item", "Allergy_Info", "Is_Safe", "Timestamp", "Status", "Subtotal", "Tax_Amount", "Tip_Amount", "Total"],
    ...
    def update_user_in_csv(self, email, updated_fields, path="data/users.csv"):
        ...
        # Clear cache and refresh in-memory staff cache
        csv_cache.clear_cache(path)
        self.load_staff_from_csv()
        return True
    ...
    def load_deals_from_csv(self, path="data/deals.csv"):
        """Load deals from CSV file with caching"""
        # Check cache first
        cached_data = csv_cache.get_cached_data(path)
        if cached_data is not None:
            self.deals = cached_data
            return
        ...
        try:
            ...
            # Update cache
            csv_cache.update_cache(path, self.deals)
        except FileNotFoundError:
            pass
        except Exception as e:
    ...
    def load_shifts_from_csv(self, path="data/shifts.csv"):
        """Load shift records from CSV with caching"""
        # Check cache first
        cached_data = csv_cache.get_cached_data(path)
        if cached_data is not None:
            self.shifts = cached_data
            return
        ...
        try:
            ...
            # Update cache
            csv_cache.update_cache(path, self.shifts)
        except FileNotFoundError:
            pass
        except Exception as e:
